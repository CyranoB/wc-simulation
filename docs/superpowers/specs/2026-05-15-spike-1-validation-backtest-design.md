...
**Parent:** PRD `wcsim` v1.4 §12.2 *Back-test*
**Work location:** `spikes/01-validation/` on branch `spike/01-validation`
...
**Binary success:** Brier score on the combined 2018 + 2022 WC match set < **0.215** for **Elo mode** (the production default), using PRD-default parameters. The threshold sits 0.005 above the PRD's AC8 target of < 0.21 as a tolerance band for the ~±0.015 sampling noise inherent in a 128-match dataset. FIFA and blend modes are measured alongside; their Brier values are reported but their pass/fail is informational, not gating.
...
5. **Knockout matches — dual-convention scoring.** The match model in PRD §5.5 predicts **90-minute outcomes**; ET and penalties are separate sampling mechanisms in the production tournament engine. Evaluating a 90-min predictor against a post-ET outcome conflates the two and slightly flatters the model on draws that broke in extra time. The spike therefore computes Brier under **both** conventions and reports both:
   - **`brier_90min`:** $y_m$ uses the 90-minute regulation score. Group-stage matches: identical to step 4. Knockout matches that went to ET: counted as 90-minute draws.
   - **`brier_post_et`:** $y_m$ uses the post-ET score; penalty winners are attributed to the post-ET winner; the "draw" outcome is impossible for knockouts.

   `brier_90min` is the **gating metric** (it is what the model actually predicts). `brier_post_et` is informational. A divergence `brier_post_et - brier_90min > 0.01` is reported as a flag for Spike 2 to investigate ET handling before the production tournament engine is built.
...
- **Brier per mode and convention:** $B_{\text{mode},\text{conv}} = \frac{1}{N} \sum_m \sum_k (\hat{p}_{m,k,\text{mode}} - y_{m,k,\text{conv}})^2$ — six values total (3 modes × 2 conventions).
...
- **Breakdowns:** Brier broken out by tournament (2018, 2022, combined), by mode, by convention, and by outcome bucket (`favourite_wins`, `underdog_wins`, `draw`) for sanity. *Favourite* is defined by higher Elo at kickoff, independent of which mode is being scored — so the buckets stay comparable across modes.
...
All committed to git on `spike/01-validation` branch.

## 6. Pass / Fail

| Outcome | Criterion (Elo mode is the gate) | Action |
|---|---|---|
The gating metric is **`brier_90min[elo].total`** (the 90-min Brier on Elo mode — what the production match model actually predicts).

| Outcome | Criterion | Action |
|---|---|---|
| **Pass** | `brier_90min[elo].total` < 0.215 **AND** Elo-mode calibration within ±0.05 of diagonal in every decile with ≥ 8 matches (sparser deciles reported but not gating) | Proceed to Spike 2. Report all 6 Brier values + draw-rate comparison alongside. |
| **Pass w/ caveat** | Elo passes, but `brier_90min[fifa].total` or `brier_90min[blend].total` > 0.225 | Proceed to Spike 2. Document the underperforming mode in PRD §11 as "exposed for comparison, not recommended as default." |
| **Soft fail** | `brier_90min[elo].total` in [0.215, 0.222] | Sweep `c-elo` ∈ [200, 400] and `μ` ∈ [1.1, 1.6]; if a setting passes, update PRD defaults and proceed. Otherwise revisit model definition. |
| **Hard fail** | `brier_90min[elo].total` ≥ 0.222 | **Stop.** Rework PRD §5.5 (candidates: multiplicative Poisson form, separate home/away $\mu$, Dixon-Coles correlation) before any further engineering. |

**Informational flags** (do not gate, but are documented in `README.md`):

- `brier_post_et[elo] - brier_90min[elo] > 0.01` → ET handling is a Spike 2 investigation item before the tournament engine is wired up.
...
Calibration-curve failure with a passing Brier counts as soft fail.
...
- ~2 hours: Brier (both conventions) + draw-rate + calibration plot + decision logic.
